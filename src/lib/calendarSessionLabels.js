/** Labels PT-BR para estados e metadados de sessões (alinhado ao backend). */

export function sessionStatusLabelPt(status) {
...
export function sessionStatusChipClass(status) {
...
export function sessionAttributionLabelPt(v) {
  const o = SESSION_ATTRIBUTION_OPTIONS.find((x) => x.value === v)
  return o?.label || v || '—'
}

export function sessionReasonLabelPt(v) {
