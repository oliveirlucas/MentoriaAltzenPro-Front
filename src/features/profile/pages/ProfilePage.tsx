...
export default function ProfilePage() {
  ...
  useEffect(() => {
    if (profile) {
      ...
      setPhone(maskPhoneBrInput(String(p.phone ?? '')))
      setCity(String(p.city ?? ''))
      setLinkedin(String(p.linkedin ?? ''))
      setGithub(String(p.github ?? ''))
      setCpf(maskCpfInput(String(p.cpf ?? '')))
      setRg(String(p.rg ?? ''))
      setBirthDate(p.birth_date ? String(p.birth_date).slice(0, 10) : '')
      setStreet(String(p.street_address ?? ''))
      setComplement(String(p.address_complement ?? ''))
      setDistrict(String(p.address_district ?? ''))
      setStateRegion(String(p.state_region ?? '')
        .toUpperCase()
        .replace(/[^A-Z]/g, '')
        .slice(0, 2))
      const cepMasked = maskCepInput(String(p.postal_code ?? ''))
      setPostal(cepMasked)
