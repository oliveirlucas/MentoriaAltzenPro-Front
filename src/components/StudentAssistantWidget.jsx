 ...
 */
function buildMentorXWelcome(firstName) {
...
/**
 * @param {boolean} darkBubble
 */
function getMarkdownComponents(darkBubble) {
  const t = darkBubble
    ? {
...
export default function StudentAssistantWidget({
  ...
  const send = useCallback(async () => {
    ...
    setInput('')
    // Histórico = turnos anteriores; a pergunta atual vai só em `message` (o back junta ao contexto).
    ...
    try {
      ...
      setMessages((m) => [
        ...
        {
          role: 'assistant',
          content: e?.message || 'Não deu certo de enviar. Dá outra tentada aí.',
        },
      ])
    } finally {
      setLoading(false)
            ...
            <div className="relative flex items-start gap-3 px-3 py-3 sm:px-4 sm:py-3.5">
              ...
              <div className="min-w-0 flex-1 pt-0.5">
                ...
                <p className="mt-0.5 text-xs leading-snug text-cyan-100/80">
                  Eu sei tudo sobre você, seu diagnóstico, plano, nota do mentor, agenda, teu perfil. Na mão, na
                  lata, sem firula.
                </p>
              </div>
            </div>
