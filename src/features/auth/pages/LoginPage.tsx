...
/**
 * Página de login. Logo em `public/logo.png` (ou ajuste `LOGO_SRC`).
 * Migrada da antiga Home.tsx para liberar a `/` para a landing page pública.
 */
...
export default function LoginPage() {
  ...
  const submit = async (e) => {
    ...
    setLoading(true)
    try {
      const d = (await login(email, password)) as { user?: { role?: string } }
    ...
    <div className="relative min-h-screen min-h-dvh overflow-hidden bg-slate-950 font-sans text-slate-100">
      ...
      <div className="pointer-events-none fixed inset-0 z-[1] min-h-[100dvh]">
        ...
        <div className="absolute inset-0 opacity-25" aria-hidden>
          <Particles id="loginParticles" init={particlesInit} options={particlesOptions as never} />
        </div>
        {!reduceMotion && (
          <motion.div
      ...
      <Link
        to="/"
        className="absolute left-4 top-4 z-20 inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-3 py-1.5 text-xs font-medium text-cyan-50/80 backdrop-blur transition hover:border-cyan-400/40 hover:bg-white/10 hover:text-white sm:left-6 sm:top-6 sm:text-sm"
      >
          ...
          <div className="w-full max-w-md">
            ...
            >
              <form onSubmit={submit} className="space-y-4 text-left">
                <div>
                  <label htmlFor="login-email" className="text-sm font-medium text-cyan-50/85">
                    E-mail
                  </label>
                  <input
                    id="login-email"
                ...
                <div>
                  <label htmlFor="login-password" className="text-sm font-medium text-cyan-50/85">
                    Senha
                  </label>
                  <input
                    id="login-password"
