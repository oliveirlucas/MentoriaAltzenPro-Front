...
function stateSortValue(row: AdminStudentListRow) {
...
const columnHelper = createColumnHelper<AdminStudentRow>()

export default function AdminStudentsDataTable({ students = [] }: { students?: AdminStudentRow[] | undefined }) {
  ...
  const programOptions = useMemo(() => {
    ...
    return [...set].sort((a, b) =>
      formatProgramType(String(a)).localeCompare(formatProgramType(String(b)), 'pt', { sensitivity: 'base' })
    )
  }, [students])
  ...
  const columns = useMemo(
    () => [
      ...
      columnHelper.accessor(
        (row) => {
          const band = computeAdminListAttention(row as AdminStudentListRow).band
      ...
      columnHelper.accessor((row) => lastActivitySortValue(row.last_form_activity), {
        ...
        sortingFn: (a, b) => {
          const av = a.getValue('last') as number | null
          const bv = b.getValue('last') as number | null
